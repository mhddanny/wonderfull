...
class OrderController extends Controller
{
    public function index()
    {
        ...
        return view('WF_Collection.orders.index', compact('orders'));
    }

    public function view($invoice)
    {
        $order = Order::with(['district.city.province', 'details', 'details.produk', 'payment'])
        ...
        //JIKA HASILNYA TRUE, MAKA KITA TAMPILKAN DATANYA
            return view('WF_Collection.orders.view', compact('order'));
        }
    ...
    {
        $order = Order::find($id);
        return view('WF_Collection.payment', compact('order'));
    }
        ...
        try {
            ...
            if ($order->status == 0 && $request->hasFile('proof')) {
                ...
                if ($request->file('proof')->isValid()) {
                  ...
                  $upload_path = 'public/uploads/customer/payment';
                  $request->file('proof')->move($upload_path,$filename);
                }
    ...
    public function pdf($invoice)
    {
        ...
        if (!\Gate::forUser(auth()->guard('customer')->user())->allows('order-view', $order)) {
            return redirect(route('WF_Collection.view_order', $order->invoice));
        }

        //JIKA DIA ADALAH PEMILIKNYA, MAKA LOAD VIEW BERIKUT DAN PASSING DATA ORDERS
        $pdf = PDF::loadView('WF_Collection.orders.pdf', compact('order'));
    ...
    public function returnForm($invoice)
    {
        ...
        return view('WF_Collection.orders.return', compact('order'));
    }
      ...
      // dd($request);
        ...
        if ($request->hasFile('photo')) {
          if ($request->file('photo')->isValid()) {
            ...
            $extention = $file->getClientOriginalExtension();
            $filename = date('YmdHis') . Str::random(5) . '.' . $extention;
            //KEMUDIAN UPLOAD KE DALAM FOLDER STORAGE/APP/PUBLIC/RETURN
            $upload_path = 'public/uploads/customer/complain';
    ...
    private function sendMessage($order_id, $reason)
    {
        ...
        if ($chat['ok']) {
            ...
            $text = 'Hai Wonderful Collection, OrderID ' . $order_id . ' Melakukan Permintaan Refund Dengan Alasan "'. $reason .'", Segera Dicek Ya!';
