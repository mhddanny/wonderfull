...
use App\Http\Resources\TransaksiDetailResouse;
use App\Keranjang;
use App\Produk;
use App\Transaksi;
...
class TransaksiController extends Controller
{
      public function add_cart(Request $request)
      {
          ...
          if ($validator->fails())
          {
            ...
            //pecarian data stok Produk
            $data_produk = Produk::find($data['kd_produk']);
            $stok_produk = $data_produk->stok;

            //mencari julah produk, atas produk itu sendiri didalam table keranjang
            $jumlah_barang_cart = Keranjang::where('kd_produk',$data['kd_produk'])->sum('jumlah');

            $stok_hasil = $stok_produk - $jumlah_barang_cart;

            //jika stok hasil lebih kecil dari jumlah yang di input maka akan menapilkan tulisan "stok barang tidak mencukupi"
    ...
    function get_detail_transaksi(Request $request)
    {
      ...
      return TransaksiDetailResouse::collection($data_transaksi_detail);
    }
}
