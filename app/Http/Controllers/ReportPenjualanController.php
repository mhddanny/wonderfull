...
class ReportPenjualanController extends Controller
{
    ...
    {
      $penjualan = TransaksiDetail::orderBy('created_at','DESC');
      return view('wonderful.report_penjualan.index', compact('penjualan'));
    }

    public function cetak_pdf()
    {
      $penjualan = TransaksiDetail::orderBy('created_at','Desc')->get();
      $pdf = PDF::loadview('wonderful.report_penjualan.report_penjualan_pdf',compact('penjualan'));
      return $pdf->stream();
    }
    ...
    public function orderReport()
    {
        ...
        //KEMUDIAN LOAD VIEW
        return view('wonderful.report_penjualan.order', compact('orders'));
    }
    public function orderReportPdf($daterange)
    {
        ...
        $pdf = PDF::loadView('wonderful.report_penjualan.order_pdf', compact('orders', 'date'));
        //GENERATE PDF-NYA
        return $pdf->stream();
    }
    public function returnReport()
    {
        ...
        return view('wonderful.report_penjualan.return', compact('orders'));
    }
    ...
    public function returnReportPdf($daterange)
    {
        ...
        $orders = Order::with(['customer.district'])->has('return')->whereBetween('created_at', [$start, $end])->get();
        $pdf = PDF::loadView('wonderful.report_penjualan.return_pdf', compact('orders', 'date'));
        return $pdf->stream();
    }
}
