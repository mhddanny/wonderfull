        ...
        <div class="col-sm-6">
          ...
          <div class="right">
            @if(Request::get('keyword'))
              <a class="btn btn-sm btn-outline-success" href="{{ route('supplier.index') }}">Back</a>
            @else
              <a class="btn btn-sm btn-outline-success" href="{{ route('supplier.create') }}"><span class="glyphicon glyphicon-plus"></span>Create</a>
            @endif
          </div>
        </div>
        <div class="col-sm-6">
                   ...
                   <tr>
                        ...
                        <td>
                          <form class="" action="{{ route('supplier.destroy',[$row->kd_supplier]) }}" method="post" onsubmit="return confirm('Apakah Anda Yakin Akan Menghapus Dara ini ? ')">
                            @csrf
                            {{ method_field('Delete') }}
                            <a class="btn btn-sm btn-outline-warning "clas="btn btn-warning" href="{{ route('supplier.edit',[$row->kd_supplier]) }}">Edit</a>
