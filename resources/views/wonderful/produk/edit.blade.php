    ...
    <div class="container-fluid">
      <div class="col-md-12">
        <div class="card card-light">
          ...
          <!-- /.card-header -->
          <!-- form start -->
          <form class="form-horizontal" method="post" autocomplete="off" action="{{ route('produk.update',[$produk->kd_produk]) }}" enctype="multipart/form-data">
            @csrf
            {{ method_field('PUT')}}
            <div class="card-body">
              ...
              <div class="form-group{{$errors->has('weight') ? 'has-error' : ''}}">
                ...
                <input type="number" class="form-control" id="weight" name="weight" value="{{ $produk->weight }}" required>
              ...
              <div class="form-group{{$errors->has('ket') ? 'has-error' : ''}}">
                ...
                <textarea class="textarea" placeholder="Place some text here" name="ket"
                          style="width: 100%; height: 200px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;" required>{{ $produk->ket }}</textarea>
                @if (@$errors->has('ket'))
